...
import {
  ArrowLeft, Pencil, User, Phone, Mail, MapPin, GraduationCap,
  Users, CreditCard, CalendarDays, DollarSign, FileText, Plus,
  Trash2, Save, X, CalendarIcon, ChevronLeft, ChevronRight, StickyNote, Loader2, MessageSquare
} from "lucide-react";
...
export default function StudentProfilePage() {
  ...
  const { data: feeHistory = [] } = useMonthlyFeeHistory(studentId);
  ...
  // ── notes ──
  const { data: notes = [], isLoading: notesLoading } = useStudentSalesNotes(studentId);
  ...
  const canDeleteNote = (note: { created_by: string }) => {
    return note.created_by === user?.id || isAdmin || isCipher;
  };
  ...
  const statusColor = student.status === "active" ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" :
  ...
  return (
    <div className="space-y-6">
      ...
      <Breadcrumb>
        <BreadcrumbList>
          <BreadcrumbItem><BreadcrumbLink asChild><Link to="/students">Students</Link></BreadcrumbLink></BreadcrumbItem>
          <BreadcrumbSeparator />
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Badge className={cn("capitalize", statusColor)}>{student.status}</Badge>
            </div>
            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
              {student.student_id_number && (
                <Badge variant="outline" className="text-xs">{student.student_id_number}</Badge>
              )}
        ...
        <div className="flex gap-2">
          {effectiveCanEdit && (
            <Button variant="outline" onClick={() => navigate(`/students/${student.id}`)}>
              <Pencil className="mr-2 h-4 w-4" /> Edit / Payments
            </Button>
          )}
        </div>
      </div>
      ...
      {/* Section 5: Sales & Follow-up Notes */}
      <Card>
        <CardContent className="pt-6">
